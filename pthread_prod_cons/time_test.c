...
void exectimes(double value, enum Function_name function_name, enum Task task){
    ...
    if(task == SET_TIME) {
        ...
        char *filename = "pthread_prod_cons_time_log.txt";
        ...
        sprintf(buffer, "##### pthread ### prod-cons ############# \n\n");
        fwrite(buffer, strlen(buffer), 1, fp);
        printf("--> Tempi di esecuzione salvati in \"pthread_prod_cons_time_log.txt e in pthread_prod_cons_time_log.csv\"\n\n");
        fclose(fp);

        //salva i tempi nel file CSV
        fp = fopen("pthread_prod_cons_time_log.csv", "a");
